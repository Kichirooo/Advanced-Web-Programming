...
app.use(bodyParser.json()); // Untuk menangani data JSON dari fetch
app.use(express.static('public')); // Pastikan folder 'public' ada untuk file statis seperti gambar
app.use(session({
  secret: 'your-secret-key', // Ganti dengan secret yang kuat dan unik
  resave: false,
...
db.connect(err => {
  ...
  const adminPass = 'adminpassword'; // Ganti password admin di sini
  bcrypt.hash(adminPass, 10, (err, hash) => {
    if (err) throw err;
    db.query('INSERT IGNORE INTO users (username, email, password, role) VALUES (?, ?, ?, ?)',
      ['admin', 'admin@example.com', hash, 'admin'], err => {
        if (err) throw err;
      });
  });
});
    ...
    bcrypt.hash(password, 10, (err, hash) => {
      ...
      db.query('INSERT INTO users (username, email, password) VALUES (?, ?, ?)',
        [username, email, hash], (err) => {
          ...
          res.redirect('/login'); // Alihkan ke halaman login setelah registrasi berhasil
        });
    });
  });
});
      ...
      bcrypt.hash(password, 10, (err, hash) => {
        ...
        db.query('INSERT INTO users (username, email, password) VALUES (?, ?, ?)',
          [username, email, hash], (err, result) => {
            ...
            res.render('login', { error: 'Registrasi berhasil! Silakan login.' }); // Stay on login page with success message
          });
      });
    });
  } else {
...
app.get('/product/:id', isAuthenticated, (req, res) => {
  ...
  db.query('SELECT * FROM products WHERE id = ?', [productId], (err, product) => {
    ...
    // Ambil 3 produk lain secara acak, pastikan berbeda dari produk saat ini
    db.query('SELECT * FROM products WHERE id != ? ORDER BY RAND() LIMIT 3', [productId], (err, recommendations) => {
      ...
      db.query('SELECT * FROM reviews WHERE product_id = ? ORDER BY created_at DESC', [productId], (err, reviews) => {
        if (err) {
          console.error('Error fetching reviews:', err);
          reviews = []; // Default ke array kosong jika ada error
        }
        res.render('detail', { 
          product: product[0], 
...
app.post('/purchase/:id', isAuthenticated, (req, res) => {
  ...
  db.query('SELECT stock FROM products WHERE id = ?', [productId], (err, results) => {
    ...
    db.query('UPDATE products SET stock = stock - ? WHERE id = ?', [quantity, productId], err => {
      ...
      db.query('INSERT INTO purchases (user_id, product_id, quantity, size, purchase_date) VALUES (?, ?, ?, ?, NOW())',
        [req.session.user.id, productId, quantity, size], err => {
          if (err) {
...
app.post('/cancel-purchase/:id', isAuthenticated, (req, res) => {
  ...
  db.query('SELECT p.purchase_date, p.quantity, pr.stock, pr.id AS product_id FROM purchases p JOIN products pr ON p.product_id = pr.id WHERE p.id = ? AND p.user_id = ? AND p.cancelled = 0', 
    [purchaseId, req.session.user.id], (err, results) => {
      if (err) {
      ...
      const timeDiff = (now - purchaseDate) / (1000 * 60 * 60 * 24); // Selisih dalam hari
      if (timeDiff > 1) {
...
app.get('/history', isAuthenticated, (req, res) => {
  ...
  `, [req.session.user.id], (err, history) => {
    if (err) {
...
app.delete('/api/reviews/:id', isAuthenticated, (req, res) => {
  ...
  db.query('SELECT * FROM reviews WHERE id = ?', [reviewId], (err, results) => {
    ...
    // Cek apakah pengguna adalah pemilik ulasan atau admin
...
app.post('/admin/delete-purchase/:id', isAdmin, (req, res) => {
  const purchaseId = req.params.id;
  db.query('SELECT p.quantity, pr.id AS product_id FROM purchases p JOIN products pr ON p.product_id = pr.id WHERE p.id = ? AND p.cancelled = 0', 
